	        ...
	        <div class="row">
	        	<div class="col-sm-10 col-sm-offset-1">
				    ...
				    <?php
				    	...
				    	$voteData = $conn->query($qry);
						foreach( $voteData as $data){
							$sql = "SELECT * FROM votes WHERE position_id='".$data['id']."' and voters_id = '".$voter['id']."'";
							$vquery = $conn->query($sql);
							if($vquery->num_rows > 0){
								?>
								<div class="text-center"  style="margin-bottom:50px;">
									<h3>Kamu sudah melakukan voting untuk <?=$data['description']?></h3>
									<a  style="background-color : #1c2986;"id="view-vote" position="<?=$data['id']?>" href="#view" data-toggle="modal" class="btn btn-flat btn-primary btn-lg">Lihat Vote</a>
								</div>
								<?php
							}
							else{
								...
								<form method="POST" id="ballotForm" action="submit_ballot.php">
									<?php
										...
										$candidate = '';
										$sql = "SELECT * FROM positions where id='".$data['id']."'";
										$query = $conn->query($sql);
										while($row = $query->fetch_assoc()){
											$sql = "SELECT * FROM candidates WHERE position_id='".$data['id']."'";
											...
											$instruct = ($row['max_vote'] > 1) ? 'Anda dapat memilih hingga'.$row['max_vote'].' kandidat' : 'Pilih salah satu kandidat';
...
$(function(){
	...
	$(document).on('click','#view-vote',function(e){
		e.preventDefault();
		let getVoteList =document.getElementsByClassName("votelist");
		for(let i = 0;i<getVoteList.length;i++){
			if(getVoteList[i].getAttribute("position") == e.target.getAttribute("position")){
				getVoteList[i].style.visibility = "visible";
			}else{
				getVoteList[i].style.visibility = "hidden";
			}
		}
	});
	...
	$('#preview').click(function(e){
		...
		if(form == ''){
			$('.message').html('Kamu harus memilih setidaknya satu kandidat');
			$('#alert').show();
		}
		else{
			$.ajax({
